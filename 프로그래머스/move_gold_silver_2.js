...
function solution(a, b, g, s, w, t) {
    ...
    for (let i=0; i<g.length; i++) {
        for (let j=0; j<g.length; j++) {
            if (smaller > time_original[j]) {
                smaller = time_original[j]
                smallerIndex = j;
            }
        } 
        fastest.push(smallerIndex);
        time_original[smallerIndex] = (10 ** 5) + 1;
        smaller = time_original[0];
        smallerIndex = 0;
    }
    // 위 과정에서 값을 변형한 unitW배열 다시 초기화 해주기
    for (let i=0; i<g.length; i++) {
        time_original = [...t];
        console.log(time_original[i]);
    }
